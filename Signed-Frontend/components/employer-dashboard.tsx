  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Deduplicate by id (just in case) and sort by most recent
        const uniqMap = new Map<string, APIJobPosting>();
        for (const it of all) uniqMap.set(it.id, it);
        ...
        if (mounted) {
          setRawJobs(uniq);
          setJobs(mapApiToDashboardJobs(uniq));
        }
      } catch (e: any) {
